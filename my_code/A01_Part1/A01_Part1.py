...
def my_main(input_folder, output_file):
    ...
    # openning the output file for writing results
    ...
    # using a dictionary to store trip counts per station start and end trips
    my_station_trips = {}

    # traversing all files and then collecting station data
    for file in my_list_of_files:
        ...
        for line in my_input_stream:
            ...
            # extracting the station names start and stop
            stations = [record[4], record[8]]

            # updating the trip counts for each station
            for index in range(2):
                station_name = stations[index]

                if station_name not in my_station_trips:
                    my_station_trips[station_name] = [0, 0]

                my_station_trips[station_name][index] += 1

        my_input_stream.close()

    # here sorting the stations alphabetically and then writing results to file
    my_sorted_stations = sorted(my_station_trips.keys())

    for station_name in my_sorted_stations:
        counts = my_station_trips[station_name]
