...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    # using a dictionary to store total trip duration and count per bike
    my_bikes_dict = {}
    ...
    for line in my_input_stream:
        ...
        if record:
            ...
            # initializing the dictionary entry if the bike_id is new
            if bike_id not in my_bikes_dict:
                my_bikes_dict[bike_id] = [0, 0]

            # updating the total duration and trip count for the bike
            my_bikes_dict[bike_id][0] += trip_duration
            my_bikes_dict[bike_id][1] += 1

    # writing results to the output stream
    for bike in my_bikes_dict:
        total_duration, total_trips = my_bikes_dict[bike]
        my_str = "universal\t(" + str(bike) + ", " + str(total_duration) + ", " + str(total_trips) + ")\n"
