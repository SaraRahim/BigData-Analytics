...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    # extracting the target bike ID from the mapper input parameters
    target_bike = my_mapper_input_parameters[0]

    # using a dictionary to store trips grouped by day
    my_trips_by_day = {}

    # traversing the input stream and processing each line
    for line in my_input_stream:
        record = process_line(line)

        # checking if the record is valid and is equal to the target bike
        if record and record[11] == target_bike:
            # extracting trip details
            start_time = record[0]
            stop_time = record[1]
            start_station = record[4]
            stop_station = record[8]

            # grouping trips by the date portion of the start time
            day = start_time[:10]
            trip_info = (start_time, stop_time, start_station, stop_station)

            # adding the trip to the equal day
            if day not in my_trips_by_day:
                my_trips_by_day[day] = []
            my_trips_by_day[day].append(trip_info)

    # sorting trips by day and writing to the output stream
    for day in sorted(my_trips_by_day.keys()):
        # sorting trips by start time within each day
        daily_trips = sorted(my_trips_by_day[day], key=lambda x: x[0])

        # formatting the trip details
        formatted_trips = [" @ ".join(trip) for trip in daily_trips]

        # joining all trips into a single formatted string
        trips_str = " @ ".join(formatted_trips)

        # writing results to the output stream
        my_output_stream.write("universal\t(" + trips_str + ")\n")
